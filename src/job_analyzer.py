...
class JobAnalyzer:
    ...
    def is_job_posting(self, text: str) -> bool:
        ...
        indicator_count = sum(1 for indicator in self.job_indicators if indicator in text_lower)
    ...
    def prepare_response(self, analysis: Dict[str, any], matching_members: Dict[str, List[str]]) -> str:
        """
        ...
        if matching_members:
            for user_id, skills in matching_members.items():
                response.append(f"• <@{user_id}> - Matching skills: {', '.join(skills)}")
        else:
            response.append("No direct matches found. Consider reaching out to brothers to develop these skills!")
